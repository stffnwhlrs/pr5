...
public class MongoDBSimpleConsumer {
    ...
    public void run(long a_maxReads, String a_topic, int a_partition,
        ...
        while (a_maxReads > 0) {
            ...
            MongoDatabase db = client.getDatabase("tweetComparison");
            MongoCollection<Document> fishCollection = db.getCollection("tweetComparisons");
            Gson gson = new Gson();
            Type type = new TypeToken<TweetComparisonEvent>() {}.getType();
            for (MessageAndOffset messageAndOffset : fetchResponse.messageSet(a_topic,
                ...
                payload.get(bytes);
                TweetComparisonEvent incomingFish = gson.fromJson(new String(bytes, "UTF-8"), type);
                System.out.println(incomingFish);
                fishCollection.insertOne(incomingFish.getTweetComparisonEventAsDocument());
                numRead++;
                a_maxReads--;
            }
            if (numRead == 0) {
